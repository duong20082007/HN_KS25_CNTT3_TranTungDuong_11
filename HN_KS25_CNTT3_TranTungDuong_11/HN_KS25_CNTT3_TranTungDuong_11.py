...
class TutoringClassManager:
    ...
    def add_class(self):
        ...
        try:
            ...
            if tuition_fee < 0: raise ValueError
            student_count = int(input("Nhập số lượng học viên (0-100): "))
            if not (0 <= student_count <= 100): raise ValueError
            operating_cost = float(input("Nhập chi phí vận hành: "))
            if operating_cost < 0: raise ValueError
        except ValueError:
...
def main():
    ...
    while True:
        ...
        if choice == '1': manager.show_all()
        elif choice == '2': manager.add_class()
        elif choice == '3': manager.update_class()
        elif choice == '4': manager.delete_class()
        elif choice == '5': manager.search_class()
        elif choice == '6':
